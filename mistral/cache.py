"""
Implementazione della rolling buffer kv-cache.
Viene implementata nel contesto dell'attenzione
con lo scopo di ottimizare l'efficienza.
"""
...
class RotatingCacheInputMetadata:
    ...
    prefill: bool
    mask: AttentionBias # Maschera per l'attenzione
    seqlens: List[int]
...
def unrotate(cache: torch.Tensor, seqlen: int) -> torch.Tensor: # seqlen = n° totale di token memorizzati finora. Questo serve per calcolare il punto di rotazione della cache rotante
    assert cache.ndim == 3  # (W, H, D) -> (sliding_window_size, num_head, head_dim)
    position = seqlen % cache.shape[0] # punto di rotazione attorno al quale dobbiamo ruotare la cache
    if seqlen < cache.shape[0]: # se la lunghezza totale della sequenza (seqlen) è più piccola della dimensione della cache
        return cache[:seqlen] # restituisce solo i primi elmenti seqlen
    elif position == 0:
        return cache
    else:
        return torch.cat([cache[position:], cache[:position]], dim=0) # seleziona gli elementi non ruotati dalla cache intorno al punto di rotazione
...
class CacheView:
    ...
    def update(self, xk: torch.Tensor, xv: torch.Tensor):
        ...
        # Copia dai tensori xk e xv ai tensori della cache, basandosi sulle posizioni della cache e sugli elementi da memorizzare nella cache (to_cache_mask)
    ...
    def interleave_kv(self, xk: torch.Tensor, xv: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        interleaved_k = interleave_list(cache_k, xk) # Aggiunge le chiavi in arrivo agli elementi attualmente memorizzati (uno per ogni prompt)
        interleaved_v = interleave_list(cache_v, xv) # Aggiunge i valori in arrivo agli elementi attualmente memorizzati (uno per ogni prompt)
...
class RotatingBufferCache:
    ...
    def get_input_metadata(self, seqlens: List[int]) -> RotatingCacheInputMetadata:
        ...
        seqpos = self.kv_seqlens.tolist() # Indica la lunghezza totale vista dalla cache finora (inclusi gli elementi sovrascritti) per ogni prompt

        assert len(seqlens) > 0, seqlens
        
        # [True] se la posizione del token appartiene agli ultimi `sliding_window` posizioni della sequenza. È sempre True a meno che la dimensione del chunk sia maggiore della finestra scorrevole
        # Indica quali elementi della sequenza dovrebbero essere memorizzati nella cache (cioè gli ultimi `sliding_window` token di ogni sequenza)
        masks = [
            [x >= seqlen - self.sliding_window for x in range(seqlen)]
            for seqlen in seqlens # La lunghezza della sequenza di ciascun input nel batch (così possiamo capire quale token appartiene a quale prompt)
        ]
        
        # Indica quali elementi della sequenza dovrebbero essere memorizzati nella cache (gli ultimi `sliding_window` token di ogni sequenza)
        # Concatena tutte le maschere di ciascun prompt nel batch
        to_cache_mask = torch.tensor(sum(masks, []), device=self.device, dtype=torch.bool)
        
        # n° di elementi nella mask == true 
        cached_elements = torch.tensor([sum(mask) for mask in masks], device=self.device, dtype=torch.long)
        
        # La posizione di ciascun token nel prompt (tutti concatenati). Potrebbe non partire da zero (perché ad esempio il primo chunk potrebbe essere composto da 5 token e stiamo ora elaborando il secondo chunk)
        ...
        # L'indice del batch a cui appartiene ciascun token (nella lista concatenata)
        batch_idx = torch.tensor(sum([[i]*seqlen for i, seqlen in enumerate(seqlens)], []), device=self.device, dtype=torch.long)
        
        # Posizione di dove ogni token dovrebbe essere posizionato nella cache (basato sulla posizione nel prompt e sull'indice del batch)
        cache_positions = positions % self.sliding_window + batch_idx * self.sliding_window

        # Indica se è il primo riempimento iniziale (vero solo sul primo chunk)
        first_prefill = seqpos[0] == 0
        subsequent_prefill = any(seqlen > 1 for seqlen in seqlens)
        # Per il primo chunk del prompt. Crea una maschera di attenzione che è causale per ciascun prompt e anche locale in base alla dimensione della finestra scorrevole
        if first_prefill:
            assert all([pos == 0 for pos in seqpos]), (seqpos)
            mask = BlockDiagonalCausalMask.from_seqlens(seqlens).make_local_attention(self.sliding_window)
        elif subsequent_prefill:
            # Per i chunck successivi
            mask = BlockDiagonalMask.from_seqlens(
                q_seqlen=seqlens, # dimensione della query
                kv_seqlen=[s + cached_s.clamp(max=self.sliding_window).item() for (s, cached_s) in zip(seqlens, self.kv_seqlens)] # Il numero totale di chiavi e valori sarà la lunghezza della sequenza in ingresso + gli elementi memorizzati
            ).make_local_attention_from_bottomright(self.sliding_window)
        else: # per la generazione di token
            mask = BlockDiagonalCausalWithOffsetPaddedKeysMask.from_seqlens(
                q_seqlen=seqlens, # Size of the query
                kv_padding=self.sliding_window,
                kv_seqlen=(self.kv_seqlens + cached_elements).clamp(max=self.sliding_window).tolist() # Il numero totale di chiavi e valori sarà la lunghezza della sequenza in ingresso + gli elementi memorizzati
            )
